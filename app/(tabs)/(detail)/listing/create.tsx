...
import {
  ...
  type ListingCondition, type Category,
} from '@/src/types';
  ...
  if (!session || (profile?.role !== 'seller' && profile?.role !== 'admin')) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.gateTitle}>Sellers only</Text>
        <Text style={styles.gateSub}>You need a seller account to post listings.</Text>
        <TouchableOpacity style={styles.greenBtn} onPress={() => router.push('/seller/apply')}>
          <Text style={styles.greenBtnText}>Become a Seller</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <>
      ...
      >
        ...
        <Text style={styles.sectionSub}>Add up to {MAX_IMAGES} photos. First photo is the cover.</Text>
...
const styles = StyleSheet.create({
  ...
  greenBtn: { backgroundColor: '#15803d', borderRadius: 12, paddingHorizontal: 28, paddingVertical: 14 },
  greenBtnText: { color: '#fff', fontWeight: '700', fontSize: 15 },
  gateTitle: { fontSize: 18, fontWeight: '700', color: '#111827' },
  gateSub: { fontSize: 14, color: '#6b7280', textAlign: 'center' },
});
